...
RUN groupadd -g 1000 fuelPriceApp && \
	useradd -ms /bin/bash -u 1000 -g 1000 fuelPriceApp && \
	mkdir -p /app && \
	chown fuelPriceApp:fuelPriceApp /app
...
COPY --chown=fuelPriceApp:fuelPriceApp package.json .
COPY --chown=fuelPriceApp:fuelPriceApp yarn.lock .

RUN yarn install --frozen-lockfile && yarn cache clean

COPY --chown=fuelPriceApp:fuelPriceApp schema.prisma .

RUN yarn prisma generate && rm -rf ~/.cache/prisma

COPY --chown=fuelPriceApp:fuelPriceApp .babelrc .
COPY --chown=fuelPriceApp:fuelPriceApp src src/
